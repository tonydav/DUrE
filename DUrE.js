...
const details = () => {
  return {
    ...
    Name: "Davo's Ultimate reEcoder", 
    Type: "Video",
    Operation: "Transcode",
    Description: "Single pass NVENC encoder. Will reecode video based on tiers and can optionally downscale 4k to 1080p. If target rate percentage is lower than tier will scale to that. Reencodes HEVC files but target percentage is always 1. Will removes audio and subtitles that are not in the configured language or marked as commentary. v6 now will give priority options if there are multiple audio tracks. E.g. truehd>dts>ac3. v7 now allows optional reencoding of audio with bitrate per channel and desired codec and number of channels. Default is still to copy the audio though.",
    Version: "11.0",
...
/*
      {
        name: "codec_preferences", // Adding codec preferences as a new input
        type: "array", // This is now an array of codec objects
        defaultValue: [
          { name: "dts-hd ma", defaultValue: '2' },
          { name: "truehd", defaultValue: '1' },
          { name: "dts-hd", defaultValue: '3' },
          { name: "dts", defaultValue: '4' },
          { name: "atmos", defaultValue: '5' },
          { name: "flac", defaultValue: '6' },
          { name: "tta", defaultValue: '7' },
          { name: "aac", defaultValue: '8' },
          { name: "eac3", defaultValue: '9' },
          { name: "ac3", defaultValue: '10' },
          { name: "mp3", defaultValue: '11' },
          { name: "vorbis", defaultValue: '12' },
          { name: "opus", defaultValue: '13' },
          { name: "wav", defaultValue: '14' },
          { name: "pcm_s16le", defaultValue: '15' },
          { name: "pcm_s24le", defaultValue: '16' },
          { name: "pcm_s32le", defaultValue: '17' },
          { name: "wma", defaultValue: '18' },
          { name: "ra", defaultValue: '19' },
          { name: "ape", defaultValue: '20' }
        ],
        inputUI: { type: "textarea" }, // Optional, depending on how the input UI handles arrays
        tooltip: `Specify codec preferences with their priority values. This allows prioritization of multiple audio tracks.`
      }
*/	  
    ],
  };
...
function buildVideoConfiguration(inputs, file, jobReport) {
  ...
  function videoProcess(stream, id) {
    ...
    // Add NVENC settings for HEVC transcoding with calculated bitrate and buffer size
    configuration.AddOutputSetting(`-c:v hevc_nvenc -qmin 0 -cq:v ${inputs.quality_target} -b:v ${bitratetarget_kbps}k -maxrate ${bitratetarget_kbps}k -bufsize ${bufferSize_kbps}k`);
    ...
    jobReport.steps.push("Transcoding to HEVC using NVidia NVENC");
...
const plugin = (file, librarySettings, inputs) => {
  ...
  const jobReport = {
    type: "plugin",
    pluginName: "DUrE - Davo's Ultimate ReEncoder",
    steps: [],
  };
  ...
  const videoSettings = buildVideoConfiguration(inputs, file, jobReport);
  ...
  response.preset = `-hwaccel cuda -hwaccel_output_format cuda <io> ${videoSettings.GetOutputSettings()} ${audioSettings.GetOutputSettings()} ${subtitleSettings.GetOutputSettings()} -max_muxing_queue_size 9999 -bf 5 -analyzeduration 2147483647 -probesize 2147483647 -map_metadata 0 -metadata DavoProcessed="true"`;
  ...
  return {
    ...
    infoLog: "", // Removed logger.GetLogData()
    report: jobReport, // Add the job report here
  };
}
